function varargout = plotshaded(data,xlmin,xlmax,ylmin,ylmax,xyratio)
% x: x coordinates
...
if size(y,1)==2 %plot shaded area
    ...
    if y(2,:) == 0
        ...
        hold on;
%         plot([xlmin xlmax],[0 0],'Color',[0.6 0.6 0.6],'LineWidth',1);  % Plot baseline
%         hold on;
%         plot([xlmin xlmin],[0 ylmax],'Color',[0.6 0.6 0.6],'LineWidth',1);  % Plot baseline
%         hold on;
        plot(x,y(1,:),'k','LineWidth',2);
        xlim([xlmin xlmax])
        ylim([ylmin ylmax])
